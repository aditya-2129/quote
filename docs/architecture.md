...
- `src-tauri/src/cad/topology.rs` exposes the native `extract_topology` Tauri command for BREP topology extraction. It calls the narrow C ABI in `src-tauri/cpp/topo_shim.cpp`, which uses local OCCT from vcpkg to read STEP bytes and return a versioned topology envelope from `src-tauri/src/cad/serialize.rs`; the payload contains JSON-serializable faces, edges, face-edge adjacency, wire loops, and analytic surface classifications from `src-tauri/src/cad/surfaces.rs`. `src/utils/topology.ts` mirrors the envelope on the JS side and builds `TopologyGraph` lookup maps for feature-recognition code. Use topology APIs for exact BREP concepts such as faces, analytic surfaces, wires, and adjacency; use mesh APIs for viewer geometry, bounding boxes, tessellated measurements, and legacy/fallback shape analysis. This is local/native only; there is no backend service. Developer setup requires `vcpkg install opencascade:x64-windows` or an equivalent `VCPKG_ROOT`; normal app users should receive the compiled app plus required OCCT DLLs.
